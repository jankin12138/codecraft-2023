    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            ...
            if (will_crash(robots[i], robots[j])) {
                ...
                double ratio = 0.3;
                extra_rotates[i] += mod_pi(rad - robots[i].pos_rad) * ratio;
                extra_rotates[j] += mod_pi(rad + pi - robots[j].pos_rad) * ratio;
            }
        }
    }
    for (int i = 0; i < n; ++i) {
...
bool RobotCrashPreventer::will_crash(Robot &robot1, Robot &robot2) {
    if (distance(robot1.pos_x, robot1.pos_y, robot2.pos_x, robot2.pos_y) > 0.53 * 3)
        return false;
    return true;
}
